...
static void __do_fork(void *aux) {
    ...
    for (int i = 0; i < FD_MAX; i++) {
        ...
        if (i == 0 || i == 1) {
            current->fd_table[i] = file; // stdin, stdout 공유
        } else {
...
*/
void setup_argument_stack(char **argv, int argc, void **rsp_ptr,
                          uint64_t *argv_addr_out) {
    uint8_t *rsp = *rsp_ptr;
    ...
    *(uint64_t *)rsp = 0;
    ...
    for (int i = argc - 1; i >= 0; i--) {
        rsp -= 8;
        *(uint64_t *)rsp = (uint64_t)arg_addresses[i];
    }

    *argv_addr_out = (uint64_t)rsp;
