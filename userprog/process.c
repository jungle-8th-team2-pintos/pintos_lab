...
static void push_args_to_stack(char **argv, int argc, struct intr_frame *if_);
 ...
 * 새 스레드는 process_create_initd()가 반환되기 전에 스케줄되거나 (종료될 수도)
 * 합니다. initd의 스레드 ID를 반환하며, 스레드를 생성할 수 없으면 TID_ERROR를
 * 반환합니다. 이 함수는 단 한 번만 호출되어야 합니다.*/
...
int process_exec(void *f_name) {
   ...
   setup_stack() 안에서 참조하도록 구현해야 합니다. */
    ...
    // hex_dump((void*)_if.rsp, (void*)_if.rsp, USER_STACK - (uint64_t)_if.rsp,
    // true);
...
struct ELF64_PHDR {
    ...
    uint64_t p_memsz; // 메모리에 필요한 전체 크기(파일보다 클 수 있음 ->
                      // 나머지는 0으로 채움)
    uint64_t p_align;
};
...
static bool load(const char *file_name, struct intr_frame *if_) {
    ...
    if (t->pml4 == NULL)
        goto done; // 생성 실패시 종료
    process_activate(
        thread_current()); // 생성된 PML4를 활성화 (CR3 레지스터 설정)
    ...
    if (file_read(file, &ehdr, sizeof ehdr) != sizeof ehdr // 헤더 크기만큼 읽기
        ...
        || ehdr.e_phentsize !=
               sizeof(struct Phdr) // 프로그램 헤더 엔트리 크기 확인
    ...
    /* 스택 설정. */
    if (!setup_stack(if_, argv,
                     argc)) // 스택 페이지 할당 및 RSP 설정 rsp (스택 시작 지점)
        goto done;
