...
void thread_init(void) {
     ...
     * 커널은 나중에 gdt_init()에서 사용자 컨텍스트를 포함한 GDT를 재구성할
     * 것임.
     */
...
void thread_unblock(struct thread *t) {
    ...
    list_insert_ordered(&ready_list, &t->ready_elem, thread_compare_priority,
                        NULL);
...
void thread_set_nice(int nice UNUSED) { /* TODO: 여기에 구현을 작성하세요 */ }
...
static void schedule(void) {
    ...
    if (curr != next) {
        /* 전환 이전에 실행되던 스레드가 종료 상태(THREAD_DYING)라면,
                ...
                실제로 파괴(해제)하는 로직은 다음 스케줄의 시작
           시점(schedule()의 시작)에서 실행됨. */
