...
void timer_init(void) {
    ...
    그런데 핀토스는 이 타이머를 초당 TIMER_FREQ번 인터럽트를 발생시키도록
    설정하고 싶어 함. (예: #define TIMER_FREQ 100이면 초당 100번 인터럽트)
...
void timer_calibrate(void) {
    ...
    /* loops_per_tick 값을 타이머 틱 하나보다 작은 값 중 가장 큰 2의
     * 거듭제곱으로 근사함 */
...
static void real_time_sleep(int64_t num, int32_t denom) {
    ...
    */
    ...
    if (ticks > 0) {
        ...
        더 정밀한 틱 이하(sub-tick) 단위의 타이밍을 위해 busy-wait 루프를
        사용함. 오버플로우를 방지하기 위해 분자(numerator)와 분모(denominator)를
        1000으로 나누어 크기를 줄임. */
        ASSERT(denom % 1000 == 0);
